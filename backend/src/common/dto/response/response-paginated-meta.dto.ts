...
export class ResponsePaginatedMetaDto {
  ...
  @ApiProperty({ description: '総件数', example: 100, required: false })
  totalItems?: number;

  @ApiProperty({ description: '現在のページ番号', example: 1, required: false })
  currentPage?: number;

  @ApiProperty({ description: '総ページ数', example: 5, required: false })
  totalPages?: number;
